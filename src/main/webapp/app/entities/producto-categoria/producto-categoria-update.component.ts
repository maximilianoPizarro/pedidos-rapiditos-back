    ...
    if (this.productoCategoria.id) {
      this.productoCategoriaService()
        ...
        .then(param => {
          this.isSaving = false;
          this.$router.go(-1);
          const message = this.$t('pedidosrapiditosApp.productoCategoria.updated', { param: param.id });
      ...
      this.productoCategoriaService()
        ...
        .then(param => {
          this.isSaving = false;
          this.$router.go(-1);
          const message = this.$t('pedidosrapiditosApp.productoCategoria.created', { param: param.id });
