...
export default class ProductoCategoria extends mixins(JhiDataUtils) {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.productoCategorias.push(res.data[i]);
            }
  ...
  public removeProductoCategoria(): void {
    this.productoCategoriaService()
      ...
      .then(() => {
        const message = this.$t('pedidosrapiditosApp.productoCategoria.deleted', { param: this.removeId });
