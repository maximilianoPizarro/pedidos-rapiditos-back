...
export default class RestauranteCategoria extends mixins(JhiDataUtils) {
  ...
  public mounted(): void {
    this.retrieveAllRestauranteCategorias();
  }
  ...
  public retrieveAllRestauranteCategorias(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.restauranteCategorias.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: IRestauranteCategoria): void {
  ...
  public removeRestauranteCategoria(): void {
    this.restauranteCategoriaService()
      .delete(this.removeId)
      .then(() => {
        const message = this.$t('pedidosrapiditosApp.restauranteCategoria.deleted', { param: this.removeId });
